...
class GSheetManager:
    ...
    def read_scored_cache(self, expected_version: str = ""):
        ...
        expected_version이 지정되면 저장된 버전과 불일치 시 (None, None) 반환 (구버전 캐시 차단)."""
        import gzip as _gz
        ...
        if res is None and self.error_msg and "Unknown GET action: read_raw" in self.error_msg:
            ...
            # read_all 반환값(List of Dict)을 read_raw 형식(List of List)으로 변환
        ...
        try:
            ...
            # 버전 불일치 시 구버전 캐시 차단 (채점 로직 변경 후 stale 데이터 서빙 방지)
            ...
            logger.info(f"[ScoredCache] 로드 완료: ts={timestamp}, ver={stored_version}, {len(json_str) // 1024}KB")
